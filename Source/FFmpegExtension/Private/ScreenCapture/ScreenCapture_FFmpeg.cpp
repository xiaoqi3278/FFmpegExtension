...
#include "ScreenCapture/ScreenCapture_FFmpeg.h"
...
void UScreenCapture_FFmpeg::PostInitProperties()
{
	...
	if (ScreenCapInfo.bAutoCapture && !HasAnyFlags(RF_ClassDefaultObject) && this->GetWorld())
	{
		...
		std::thread CaptureThread(&UScreenCapture_FFmpeg::CaptureFunction, this);
		CaptureThread.detach();
	}
}

void UScreenCapture_FFmpeg::CaptureFunction()
{
...
bool UScreenCapture_FFmpeg::InitFormatContext()
{
...
bool UScreenCapture_FFmpeg::InitDecoder()
{
...
bool UScreenCapture_FFmpeg::InitEncoder()
{
	AVDictionary* Options = NULL;
...
bool UScreenCapture_FFmpeg::InitOutputStream()
{
	//初始化输出解封装上下文
...
bool UScreenCapture_FFmpeg::InitSwsContext()
{
...
void UScreenCapture_FFmpeg::closeCapture()
{
	bRun = false;
}

void UScreenCapture_FFmpeg::BeginDestroy()
{
...
UScreenCapture_FFmpeg::~UScreenCapture_FFmpeg()
{
	closeCapture();
}
