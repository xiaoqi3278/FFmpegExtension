...
void UStreamingServer_FFmpeg::PostInitProperties()
{
	...
	if (StreamingInfo.bAutoStreaming && !HasAnyFlags(RF_ClassDefaultObject) && this->GetWorld())
	{
		...
		std::thread StreamingThread(&UStreamingServer_FFmpeg::StreamingFunction, this);
		StreamingThread.detach();
	}
}

void UStreamingServer_FFmpeg::StreamingFunction()
{
...
void UStreamingServer_FFmpeg::OpenStreaming()
{
	std::thread StreamingThread(&UStreamingServer_FFmpeg::StreamingFunction, this);
	StreamingThread.detach();
}

void UStreamingServer_FFmpeg::CloseStreaming()
{
	bRun = false;
}

void UStreamingServer_FFmpeg::BeginDestroy()
{
...
UStreamingServer_FFmpeg::~UStreamingServer_FFmpeg()
{
	CloseStreaming();
}
